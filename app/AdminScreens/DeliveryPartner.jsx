...
export default function DeliveryPartner() {
  ...
  useEffect(() => {
    const fetchDeliveryPartners = async () => {
      try {
        const response = await axios.get('http://192.168.0.105:3500/Adminstore/delivery/getDeliveryPerson');
  ...
  const handleApprove = async (id) => {
    try {
      await axios.post(`http://192.168.0.105:3500/Adminstore/delivery/updateStatus/${id}`, {
        status: 'ACTIVE'
      });
