...
import { Keyboard, KeyboardAvoidingView, StyleSheet, View, Alert, Platform, Image } from 'react-native';
...
function LoginScreen() {
  ...
  const checkExistingCredentials = async () => {
    try {
      ...
      if (storedCreds) {
        const { phoneNumber } = JSON.parse(storedCreds);
        navigateToAppropriateScreen(phoneNumber);
      }
    } catch (error) {
  ...
  const navigateToAppropriateScreen = (phone) => {
    switch(phone) {
      case '12345':
        navigation.replace('AdminHome');
        break;
      case '0':
        navigation.replace('DeliveryTab');
        break;
      case '1':
  ...
  if (!initialCheckDone) {
    return <ActivityIndicator size="large" color="#F8931F" style={styles.loader} />;
  }
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.innerContainer}>
          <View style={styles.formContainer}>
            ...
            <Image source={loginImages.boneless} style={styles.boneless} resizeMode='contain' />
            <Image source={loginImages.logo} style={styles.logo} />
            <Image source={loginImages.leg} style={styles.leg} resizeMode='contain' />
          </View>
          
          <Animated.View 
            style={[styles.circleContainer, { transform: [{ translateY: circleAnimation }] }]}
          >
